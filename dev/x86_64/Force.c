...
#define alpha alpha_Force
 extern double alpha( double );
 /* declare global and static user variables */
#define dur dur_Force
 double dur = 100;
#define del del_Force
 double del = 10;
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 7, 2);
 	...
 	ivoc_help("help ?1 Force /home/heitor/Dropbox/Unicamp/reproducible/dev/x86_64/Force.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static char *modelname = "Force.mod ";
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  check();
 if(error){fprintf(stderr,"at line 39 in file Force.mod:\n        SOLVE check METHOD after_cvode\n"); nrn_complain(_p); abort_run(error);}
